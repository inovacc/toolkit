...
func clusterBlocksLiteral(data []byte, length uint, num_blocks uint, block_ids []byte, split *blockSplit) {
	...
	var histograms []histogramLiteral = make([]histogramLiteral, brotliMinSizeT(num_blocks, histogramsPerBatch))
	...
	for i = 0; i < num_blocks; i += histogramsPerBatch {
		...
		for j = 0; j < num_to_combine; j++ {
			...
			histograms[j].bitCost = populationCostLiteral(&histograms[j])
